...
이 스킬은 competitor-researcher → research-verifier → report-writer 3단계 에이전트 파이프라인을
"UX·AI 제품 동향" 조사에 맞게 스코핑하는 브리프 템플릿이다. 매번 새로 브리프를 쓰지 않고 아래
틀에 조사 대상만 채워서 파이프라인을 돌린다.
...
1. **브리프 작성**: 위 템플릿에 실제 조사 대상(회사명)과 기간(예: 최근 12개월)을 채워 하나의 리서치
   브리프 텍스트를 만든다.
2. **competitor-researcher 에이전트**에 브리프를 그대로 전달해 리서치를 실행한다 (출처 URL 필수,
   불확실한 정보는 "unverified"로 표기하도록 지시 — 에이전트 자체 지침에 이미 포함되어 있음).
3. 결과를 **research-verifier 에이전트**에 전달해 각 claim을 재검색·교차검증한다.
4. 검증된 결과를 **report-writer 에이전트**에 전달해 최종 보고서(md, 필요 시 pptx)로 정리한다.
...
- 이 스킬은 조사 스코프 정의용이다 — 실제 웹 리서치·검증·보고서 작성은 항상 3단계 에이전트
  파이프라인(competitor-researcher → research-verifier → report-writer)에 위임한다.
